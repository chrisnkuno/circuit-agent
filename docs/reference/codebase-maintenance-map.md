...
> **Update — 2026-08-25.** A structural pass acted on four of this map's own prescriptions. Rows
> that are now closed are marked **[closed]** below and kept rather than deleted, because the
> reasoning is what makes the next such decision cheap:
>
> - The Tauri product moved to its own repository,
>   [chrisnkuno/nova-desktop](https://github.com/chrisnkuno/nova-desktop), consuming
>   `@circuit-nova/nova-core` from npm. Its `../../../../packages/nova-cli/src/pty/*` test import
>   went with it — the stub is vendored there, since it depends on nothing but node builtins.
> - The root `app`/`components`/`lib`/`convex` product moved together to `apps/web`. This map
>   proposed the name `apps/control-plane-web`; `apps/web` was chosen instead, because with the
>   desktop gone and the root cleared there is only one, and the shorter name is unambiguous.
> - `cloudflare/circuitnotion-relay` moved to `services/circuitnotion-relay`, naming the role
>   rather than the vendor.
> - `scripts/` was regrouped as `tooling/{build,release,bench,reliability,defender}`, each entry
>   point still exposed as a stable root `package.json` alias.
>
> The move surfaced one latent defect this map had not caught: both the desktop app and the root
> suite were resolving `@anthropic-ai/sdk` — an *optional* peer of nova-core — only because the
> root `package.json` happened to depend on it. Both now declare it directly.
...
relay. See `docs/guides/defender-brain-pipeline.md`.
...
```text
apps/web                     nova-desktop (own repo)  packages/nova-cli
          |                         |                       |
          +-------------------------+-----------------------+
                                    |
...
| 7/10 **[closed]** | `apps/nova-desktop` | Tauri UI and Nova sidecar host | Product is correctly separated physically, but the sidecar assembles runtime concerns and its tests reach into CLI PTY fixtures | Depend on public `nova-engine` test helpers; remove `../../../../packages/nova-cli/src/pty/*` imports |
...
| 6/10 **[closed]** | `scripts`, `reliability`, root release config | Builds, packaging, scheduled evidence | Product builds and generated reliability-site publication are mixed in one flat script directory | Group as `tooling/{build,release,reliability}` and expose root scripts only as stable aliases |
...
| 5/10 | Defensive Brain distribution | Hosted signed feed, scheduled research queue, and global CLI replicas | The boundary, configuration health, and first trust root are explicit; rotation drills and production-feed monitoring are the next maintenance work | Keep publishing in control-plane, review in git, verification in shared core, scheduling in each front end, and retrieval in native state |
| 4/10 **[closed]** | `cloudflare/circuitnotion-relay` | Independently deployed model relay | Correctly isolated, but `cloudflare/` describes a vendor rather than the service role | Move to `services/circuitnotion-relay`; keep its own tests and deployment config |
...
- ~~`probe.mjs`, `probe-sidecar.mjs`, `mobile-check.mjs`~~ — **deleted 2026-08-25**. All three were
  one-off probes with paths hard-coded to one machine, and `probe-sidecar.mjs` pointed at a sidecar
  that now lives in another repository;
