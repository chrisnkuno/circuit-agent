...
Audited 2026-08-22 against `apps/nova-desktop` (React + Tauri shell, `sidecar/` Node host driving
`@circuit-nova/nova-core`) and compared with what `packages/nova-cli` already does. Every gap below
...
| 3 | **No background work.** The CLI has `/detach`, `/jobs`, `/watch`, `/attach` and durable job state; the sidecar has no job handlers at all, so the desktop can only run what the front tab is watching. | `sidecar/src/host.ts:164-244` (no job case), `packages/nova-cli/src/jobs-command.ts` | Start work, close the window, reopen: the job is still running and its output is attachable; approvals raised by a background job reach the window |
...
| 7 | **Files are read-only.** The CLI has a built-in editor (`/edit`) that writes through the workspace, sandbox included; the desktop can only view. | `sidecar/src/host.ts:233` (no write case), `packages/nova-cli/src/editor-screen.tsx` | A file edited in the window is written through the workspace and appears in the next diff |
| 8 | **What the agent can call is invisible.** No desktop equivalent of `/tools`: skills, hooks, MCP servers and plugins loaded from `.nova/` are not listed anywhere in the UI. | `packages/nova-cli/src/tools-command.ts` vs `apps/nova-desktop/src` (no tools panel) | A panel lists tools, providers and hook scripts for the session, built from the same call the turn uses |
...
| 11 | **No way to pay.** The CLI now has `/pay` (top-up, balance, `status <ref>`) against a billing gateway; the desktop has no payment surface at all, and a window is the easier place to show a checkout. | `packages/nova-cli/src/pay.ts` vs `apps/nova-desktop` (no billing IPC) | The window creates a checkout, shows the link and code, confirms the payment, and shows the balance the service reports |
| 12 | **Voice and `/wander` are CLI-only.** Both exist in the CLI and have no desktop surface; the window is where a microphone is actually convenient. | `packages/nova-cli/src/voice.ts`, `packages/nova-cli/src/wander.ts` | Voice capture produces an editable prompt in the composer |
...
- **Environment awareness before the first command.** The agent now probes what is actually installed
  ...
  `packages/agent-core/src/nova-cli/environment.ts`, probed once per session in `agent.ts`.
- **A missing program is reported as one.** `run_command` now names the program and tells the model
  ...
  `packages/agent-core/src/nova-cli/tools.ts` (`missingProgram`).
- **Paying from the CLI.** `/pay <amount>` tops up Nova credit through a provider-agnostic billing
  ...
  computed. `packages/agent-core/src/nova-cli/billing.ts`, `packages/nova-cli/src/pay.ts`.
- **`Unsupported model finish reason: null` fixed.** An unstated finish reason is read from the
  ...
  unrecognised *word* still errors. `packages/agent-core/src/providers/openai-compatible.ts`,
  `circuitnotion.ts`.

`/todos` and `/detach` were checked and are implemented in the CLI (`packages/nova-cli/src/commands.ts:34,53`,
