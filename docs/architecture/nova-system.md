...
Companion documents: `docs/architecture/hosted-platform.md` (the hosted platform's contracts), `docs/planning/gap-register.md`
(what is implemented versus verified), `docs/reference/optimization-map.md` (performance budgets and their
probes). The desktop app's own audited gaps moved with it to
...
**nova-desktop** (†) — a Tauri shell over a Node sidecar that hosts the same engine and speaks a
line-delimited JSON protocol over stdio. It lives in its own repository,
[chrisnkuno/nova-desktop](https://github.com/chrisnkuno/nova-desktop), and consumes this one's
`@circuit-nova/nova-core` from npm. It is drawn here because it is part of the system even though
it is no longer part of this repository.

**`apps/web/convex/`, `apps/web/lib/`** — the hosted platform: durable dispatcher, budget reservation and settlement,
...
carry their own session, model and cost. It is maintained separately in
[chrisnkuno/nova-desktop](https://github.com/chrisnkuno/nova-desktop), where its audited gaps are
tracked.
