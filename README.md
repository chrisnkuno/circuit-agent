...
The full production coding-dispatch path has completed live, end-to-end runs against real providers: `CodingAgentWorker` with `CircuitNotionCodingModelProvider` and a real E2B sandbox completed a plan-write-verify cycle, and the newer `IterativeCodingAgentWorker` with `CircuitNotionAgentTurnProvider` completed a 5-turn, 5-tool-call loop that wrote a source file and a passing test — both called directly from a developer machine. The Convex dispatcher now runs on a one-minute cron, so a queued coding run executes automatically rather than requiring a manual trigger. Repeating that dispatch through the deployed `/terminal` UI (real sign-up, real task, real budget authorization, real worker claim) surfaced that CircuitNotion's Cloudflare protection blocks the identical call when it originates from Convex Cloud's outbound IP instead of a developer machine — the request never gets a JSON response, so the step fails closed with 0 RWF spent rather than a false success. See the Model gateway row in [docs/gap-register.md](docs/gap-register.md) for the exact finding and next step. The Google Calendar control plane, vault, webhook, and schedule worker are deployed to the Convex development environment. Final user-consent verification remains blocked until a Google Cloud OAuth web client ID and secret are configured; every other catalogue connector remains inactive. GitHub repository provisioning is built and unit-tested but has no registered App yet. Production readiness still requires migrating the dispatcher to the iterative worker, resolving the CircuitNotion egress block (or an OpenAI fallback), real payment authorization, durable repository cloning into the E2B workspace, and provider-specific research/writing workers.

## CircuitNotion relay (Cloudflare Worker)

CircuitNotion's Cloudflare protection currently blocks Convex Cloud's shared outbound IP range, so a live coding run dispatched through the deployed Convex action fails even though the identical call succeeds from a developer machine (see the Model gateway row in [docs/gap-register.md](docs/gap-register.md)). [cloudflare/circuitnotion-relay](cloudflare/circuitnotion-relay) is a small reverse-proxy Worker that gives the call a different egress path; see its own README for deploy steps and how to point `CIRCUITNOTION_BASE_URL`/`CIRCUITNOTION_RELAY_SECRET` at it.
