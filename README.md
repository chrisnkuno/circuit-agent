...
- [ ] **Webhook-heavy connectivity** — Push-based fanout between system components (sandbox probes, Telegram, Resend, optional outbound org webhooks) so components stay in sync without relying only on polling/crons; signed inbound system webhook for component status callbacks. Run lifecycle itself is now push-driven internally: a completed step, a granted approval, and a scheduled retry each wake the dispatcher directly instead of waiting for its cron.
