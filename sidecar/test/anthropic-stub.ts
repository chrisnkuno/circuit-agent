 ...
 *
 * The agent runtime always sends `stream: true` (streaming is unconditional in nova-core), so a
 * plain JSON response is not enough: the `@anthropic-ai/sdk` client would fail to parse it. Every
 * event this emits mirrors the subset `collectAnthropicStream` reads.
 *
 * Vendored when this app was split out of the circuit-agent monorepo. Upstream is
 * `packages/nova-cli/src/pty/anthropic-stub.ts` in https://github.com/chrisnkuno/circuit-agent,
 * kept in sync by hand: it is a test double depending on nothing but node builtins, so a copy is
 * cheaper than a published test-only package. If the real SSE shape it mirrors changes, both
 * copies have to change, and this note is the only thing that says so.
 */
