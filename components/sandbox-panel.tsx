...
import { Box, Pause, Play, Trash2 } from "lucide-react";
...
export function SandboxPanel({ organizationId }: { organizationId: Id<"organizations"> | undefined }) {
  ...
  const hasSandboxes = Boolean(sandboxes && sandboxes.length > 0);
  ...
  if (!hasSandboxes && !hasUsage) {
    return (
      <SidePanel
        title="Sandboxes"
        icon={Box}
        count={0}
        empty={<p className="side-panel-empty">Live E2B workspaces appear here while a coding run holds them.</p>}
      />
    );
  }

  return (
    <SidePanel title="Sandboxes" icon={Box} count={sandboxes?.length ?? 0}>
